...
def classification_error(x, y, weights, b):
    p = x.shape[0]
    y_val = activation(np.dot(x, weights) + b)
    err = np.absolute(y - sgn(y_val))
    t = float(1) / (2 * p)
    return t * np.sum(err)


def energy_fn(x, y, w, b):
    y_val = activation(np.dot(x, w) + b)
    err = y - y_val
...
def train(X, Y, N, M):
    ...
    weight_interval = [-1, 1]
    w = (weight_interval[1] - weight_interval[0]) * \
        np.random.random((N, M)) + weight_interval[0]

    # Initialize bias with mean 0 in interval [-1, 1]
    bias_interval = [-1, 1]
    b = (bias_interval[1] - bias_interval[0]) * \
        np.random.random((1, M)) + bias_interval[0]

    for iteration in xrange(iterations):

        # Get a random pattern and its associated target
        length = X.shape[0]
        randindex = int(np.floor(np.random.random_sample() * length))
        x = X[randindex, :].reshape(1, N)
        y_actual = Y[randindex, :].reshape(1, M)
        ...
        w += delta_w
        if iteration % 100 == 0:
            s_train.append(energy_fn(X, Y, w, b))
...
for experiment in range(10):
    ...
    storage_classification.append([classification_error(X, Y, w),
                                   classification_error(X_val, Y_val, w)])
    storage_training.append(s_train)
    storage_validation.append(s_val)
